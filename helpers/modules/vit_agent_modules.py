...
class EncoderGreedy(nn.Module):
    def __init__(self, params):
        ...
        hparams: if_pos_enc
        """
        super(EncoderGreedy, self).__init__()
        self.params = params
        self.patch_embed = nn.Conv3d(in_channels=self.params["in_channels"] * 9, out_channels=self.params["d_model"],
                                     kernel_size=self.params["patch_size"], stride=self.params["patch_size"])
        self.pos_emb = nn.Linear(3, self.params["d_model"])
        self.transformer_encoder = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=self.params["d_model"],
                                                                                    nhead=self.params["nhead"],
                                                                                    dim_feedforward=self.params[
                                                                                        "dim_feedforward"],
                                                                                    dropout=self.params["dropout"]),
                                                         num_layers=self.params["num_layers"],
                                                         norm=nn.LayerNorm(self.params["d_model"]))

    def forward(self, X_small, X_large, X_pos, X_next_pos):
        # X_small: (T, B, 1, P, P, P), X_large: (T, B, 1, 2P, 2P, 2P), X_pos: (T, B, 3), X_next_pos: (1, B, 3)
        X_patch_emb = self.emb_patch(X_small, X_large)
        X_pos_emb = self.pos_emb(X_pos)  # (T, B, d_model)
        X_next_pos_emb = self.pos_emb(X_next_pos)  # (1, B, d_model)
        X_emb = torch.cat([X_patch_emb + X_pos_emb, X_next_pos_emb], dim=0)  # (T + 1, B, d_model)
        X_emb = self.transformer_encoder(X_emb)  # (T + 1, B, d_model)
        X_emb = X_emb[-1, ...]  # (B, d_model)

        return X_emb

    def emb_patch(self, X_small, X_large):
        # X_small: (T, B, 1, P, P, P), X_large: (T, B, 1, 2P, 2P, 2P), X_pos: (T, B, 3)
        T, B, C_in, P = X_small.shape[:4]
        assert P == self.params["patch_size"] and C_in == self.params["in_channels"]
        X_large = X_large.reshape((T, B, C_in, 2, P, 2, P, 2, P))
